...
/**
 * TODO: 自定义的可以自动换行的TextView
 */
public class AutoTextView extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            int leftc = left + params.leftMargin;
            int rightc = childView.getMeasuredWidth() + leftc;
            int topc = top + params.topMargin;
            int bottomc = childView.getMeasuredHeight() + topc;
            childView.layout(leftc, topc, rightc, bottomc);
            //超过宽度则换行
            if (rightc + childView.getMeasuredWidth() > getMeasuredWidth()) {
                left = 0;
                top = bottomc + params.bottomMargin;
            } else {
                left = rightc + params.rightMargin;
            }
        }
